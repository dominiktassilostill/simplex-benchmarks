...
public:
  MultigridPreconditioner(SystemMatrixType &opppppp)
  {
    ...
    // c-MG
    {
      ...
      const FE_SimplexP<dim> fe(level_degrees[0]);
    ...
    for (unsigned int i = 0; i < coarse_grid_triangulations.size(); ++i)
      {
        auto &dof_handler   = dof_handlers[l];
        {
          const FE_SimplexDGP<dim>  fe(level_degrees[0]);
    ...
    // p-MG
    for (unsigned int i = 1; i < level_degrees.size(); ++i)
      {
        auto &dof_handler   = dof_handlers[l];
        {
          const FE_SimplexDGP<dim> fe(level_degrees[i]);
    ...
    for (unsigned int level = minlevel; level <= maxlevel; ++level)
      {
        ...
        if(level == 0)
        {
          ...
          mg_matrices[level].reinit(MappingFE<dim>(FE_SimplexDGP<dim>(1)), dof_handlers[level], quadrature, dummy, false, numbers::invalid_unsigned_int, false);
        }
        else if(false)
        {
          AffineConstraints<number> constraints;
        ...
        else  
        {
          ...
          mg_matrices[level].reinit(MappingFE<dim>(FE_SimplexP<dim>(1)), dof_handlers[level], quadrature, dummy, true, numbers::invalid_unsigned_int, false);
        }      
      }
