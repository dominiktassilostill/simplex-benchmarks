 ...
 * program.
 * This multigrid version first uses the auxiliary continuous space, then goes to coarser 
 * grids and finally employs polynomial coarsening to reach coarser levels.
 * See CG/cell_batched/application_hp.cc for reference.
*/
...
public:
  ...
  vmult(VectorType &dst, const VectorType &src) const
  {
    if (is_dg)
      this->matrix_free.loop(
        &Operator::do_cell_integral_manual_dg,
    ...
    {
      this->matrix_free.loop(
        &Operator::do_cell_integral_manual_cg,
        &Operator::do_face_integral_manual_cg,
        &Operator::do_boundary,
        this,
        dst,
        src,
      ...
      //   &Operator::do_cell_integral_manual_cg, this, dst, src, true);
  ...
  rhs(VectorType &dst) const
  {
    ...
    if (is_dg)
      this->matrix_free.loop(
        &Operator::rhs_range,
        &Operator::rhs_face_range,
        &Operator::rhs_boundary_range,
        this,
        dst,
        dummy,
        true,
    ...
    if (is_dg)
      MatrixFreeTools::compute_diagonal<dim, -1, 0, 1, number, VectorizedArray<number>>(
        ...
        [&](auto &phi_m, auto &phi_p) {
          do_face_integral_local(phi_m, phi_p);
        },
        [&](auto &phi) { do_boundary_local(phi); },
        0,
        0,
        0);
    else
...
private:
    ...
    for (unsigned int cell = range.first; cell < range.second; ++cell)
      {
        ...
        for (unsigned int q = 0; q < integrator.n_q_points; ++q)
        {
          const auto g = evaluate_function(sol, integrator.quadrature_point(q));
          
          integrator.submit_normal_derivative(-g, q);
          integrator.submit_value(2. * sigma * g, q);
        }
  ...
  do_face_integral_local(
     FEFaceIntegrator &integrator_inner,
     FEFaceIntegrator &integrator_outer) const
  {
      {
        
        integrator_inner.evaluate(
                                         EvaluationFlags::values |
                                           EvaluationFlags::gradients);
        
        integrator_outer.evaluate(
                                         EvaluationFlags::values |
                                           EvaluationFlags::gradients);
              ...
              const std::pair<unsigned int, unsigned int> &range) const
  {
    ...
    for (unsigned int face = range.first; face < range.second; ++face)
      {
        integrator_inner.reinit(face);
        integrator_inner.gather_evaluate(src, EvaluationFlags::values | EvaluationFlags::gradients);
        ...
        for (unsigned int q = 0; q < integrator_inner.n_q_points; ++q)
          ...
          integrator_inner.read_cell_data(array_penalty_parameter)
            ...
            integrator_inner.submit_value(2. * sigma * u_inner - normal_derivative_inner, q);
          }
  ...
  void
  do_boundary_local( FEFaceIntegrator & integrator_inner) const
  {
...
public:
  MultigridPreconditioner(SystemMatrixType &opppppp)
  {
    ...
    for (unsigned int l = 0; l < n_p_levels-1; ++l)
      {
        auto &dof_handler   = dof_handlers[l];
        {
          const FE_SimplexP<dim>  fe(level_degrees[l]);
          dof_handler.reinit(*coarse_grid_triangulations[0]);
          dof_handler.distribute_dofs(fe);
        }
      }
    // h-MG
    const unsigned int max_loop_it = maxlevel;
    for (unsigned int i = 0, l = n_p_levels-1; l < max_loop_it; ++l, ++i)
      {
        auto &dof_handler   = dof_handlers[l];
        {
    ...
    // c-MG
    {
      ...
      const FE_SimplexDGP<dim> fe(level_degrees[level_degrees.size() - 1]);
    ...
    for (unsigned int level = minlevel; level <= maxlevel; ++level)
      {
        ...
        // here do DG else do CG
        if(level == maxlevel)
        {
          AffineConstraints<number> dummy;
          dummy.close();

          mg_matrices[level].reinit(MappingFE<dim>(FE_SimplexDGP<dim>(1)), dof_handlers[level], quadrature, dummy, true, numbers::invalid_unsigned_int, false);
        }
        else if(false)
        {
          AffineConstraints<number> constraints;
          ...
          //  mapping, dof_handlers[level], 0, Functions::ZeroFunction<dim>(1), constraints);
          constraints.close();

          mg_matrices[level].reinit(mapping, dof_handlers[level], quadrature, constraints, false, numbers::invalid_unsigned_int, false); 
        }
        else  
        {
          AffineConstraints<number> dummy;
          ...
          mg_matrices[level].reinit(MappingFE<dim>(FE_SimplexP<dim>(1)), dof_handlers[level], quadrature, dummy, false, numbers::invalid_unsigned_int, false);
        }      
      }
